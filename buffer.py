...
class Batch:
    ...
    def append(self, state, action, reward, done, log_pi, value, seed_ids):
        task_ids = []
        for i, seed in enumerate(seed_ids):
            if seed not in self.unique_levels.keys():
                self.unique_levels[seed] = self.unique_level_counter
                self.unique_level_counter += 1
            task_ids.append(self.unique_levels[seed])
    ...
    def get(self):
        ...
        batch = (jnp.array(self.states[:-1]), jnp.array(self.actions[:-1]), jnp.array(self.rewards[:-1]),  jnp.array(self.log_pis_old[:-1]),
                 jnp.array(self.values_old[:-1]), jnp.array(target), jnp.array(gae), jnp.array(self.task_ids[:-1]))
        return batch